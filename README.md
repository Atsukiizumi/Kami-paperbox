...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/version-0.10.8-e8dfd2?style=flat-square&labelColor=0e0d0c" alt="version 0.10.8"></a>
...
- 镜像默认要登录应用账号：首次打开先注册一个邮箱账号。不想开账号，把 `docker-compose.yml` 里 `VITE_AUTH_ENABLED` 改成 `"false"` 再重新构建，届时改用上面的配对令牌方式。为 `fasle` 时，首次打开会提示「需要配对令牌」：把启动终端里 `[kami] 局域网访问令牌` 那一串贴进去，或直接点终端里给出的配对链接（`#pair=…`）。

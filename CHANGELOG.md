...
- 多页作品并行下载：收藏多图作品时页任务并行拉取（对源站同源在飞上限 6 条不变），大批量收藏耗时明显缩短；单页失败会注明「第 N/M 页」，不跳页、由队列重试兜底。
- 队列页失败分类与一键重试：失败项按「上游限速 / 要登录订阅 / 内容不可用 / 网络失败 / 其他」聚合显示，「重试全部失败」一次回炉全部可重试项（需要登录的判死项不回炉、灰显计数）。
- 上游限速自动降档：遇到 429/503 时全局并发自动降 2 档（下限 1）冷却 90 秒，到期自动恢复设置值——不再硬顶着风控打。
- 动图 GIF 合成移到后台线程：大批动图收藏时页面不再卡顿；合成失败自动回退原路径。
- Vercel 部署路径正式移除：官方形态收敛为本机 + Docker。
- 追踪页能订阅标签看更新了（Pixiv / 三图站）：水位追踪、红点合计、跨设备同步；浏览页搜索时一键订阅当前关键词。
- 追踪页新增「今日更新」聚合视图：追踪画师与订阅标签的新作品按来源分组直接铺成瀑布流，组级一键标已读——订阅的价值从「数字」变「看图」。
- 设置 → 存储新增「补全档案」：对缺 AI / R-18 标记的存量藏品逐条拉详情回填（可取消、可断点续跑），补全后筛选笺对全量生效。
- 浏览页键盘流：J/K 移动焦点、S 收入纸匣、L 红心、E 展开预览（仅桌面，输入时让路）。
- 浏览卡对看过的作品打「看过」小标；纸匣卡不打（有自己的未读体系）。
- 统计页新增「口味变迁」：今年 vs 去年的标签 / 画师对比（新进 / 退场 / 增减）。
- 顶栏新增「遮盖敏感封面」开关：一键模糊 R-18 / AI 封面（浏览、纸匣、追踪聚合流、详情大图），Esc 快速关闭，详情页逐张点击可临时查看；会话级开关，刷新即复原。

- 案头只属于 Pixiv：FANBOX / Yande / Konachan / Danbooru 不再有案头——切到这四站直接落在浏览页，打开首页也会自动进浏览。四站的今日报纸随案头一并退场（榜单仍可在「热榜」看）。
- 今日报纸改成「贪吃蛇式」流动：榜单图化作一串小矩形节点首尾相连，蛇头带路从左向右平滑游动、后面的节点依次跟随并逐级变淡形成尾迹，游出右侧后从左侧无缝回卷；蛇身长短随当日榜单图片量伸缩，图片多时按环换班让全部名次轮流当蛇头。悬停会停下。系统开了「减弱动态效果」时显示为静态一行（无滚动条）。
- R-18 开关按站点各管各的了：设置 → 浏览把「R-18 内容」拆成 Pixiv / FANBOX / Yande / Konachan / Danbooru 五行，浏览页右上角的开关只切换**当前站点**，换站互不牵连；「过滤 AI 作画」标注为 Pixiv 专属。旧设置里的全局开关会原样带到五个站点，旧备份文件导入不受影响。

### 新增

- 设置 → 存储新增「关于」卡：显示当前版本，检测到 GitHub 上有新版本标签时提示升级并附一行 `docker compose pull && docker compose up -d`（本机 node 形态只显示版本）。

- 启动时检查下载文件夹：设置里选过文件夹、也开着「写入文件夹」，但这台浏览器访问不到它（换了浏览器 / 新设备没授权、权限被重置、文件夹被删掉或移走）时，会弹一条提示说明原因，点「去设置」直达存储分区重新选择——不再等你翻收藏库才发现图片一直悄悄存在浏览器里。
- 卡牌会挂 R-18 / R-18G 角标了：Pixiv 与 FANBOX 的成人向作品和图站一样在卡牌左下角显示分级角标（与价格同行，R-18G 单独标注）。
- 收藏匣更认得 AI 作品：入匣时保存 Pixiv 的 AI 标记，收藏卡牌的「AI」角标从此可靠显示；更早入匣的旧藏品仍按标签词识别。

### 修复

- 子代理分域审查揪出并热修六项（#171）：动图 GIF 的 Worker 路径此前在真实浏览器必然崩溃、被静默回退掩盖（「合成不卡 UI」实际零生效）——改用 OffscreenCanvas 双作用域打通；版本接口的 4 条单测因测试脚本 glob 盖不到而从未执行（已修接线）；多页收藏坏 1 页时其余页不再白下整本；429 限速期间条目重试不再提前烧完预算批量判死；发版镜像构建恢复依赖层缓存；已删除作品不再被「重试全部失败」白跑三次。

- 修掉一个刚上线就被自查揪出的键盘流缺陷：快捷键的动作信号原本会作用到整页卡片（按一次 S 把整页收进队列、按一次 L 给整页作品点真红心）；现在只作用于当前焦点卡，翻页后也不会重放旧动作。全局 Esc 统一归「关闭遮盖」。

- 修掉一个偶发整页重画的竞态：登录状态查询抢在页面首帧渲染完成之前返回时，右上角账号入口和设置页账号卡的「读取中…」文案会与页面初始内容对不上，触发 React 水合错误并导致整树重画。
...
- 标签计数改成「按张去重」口径：统计标签云、年度报告和统计页小结句里的「N 张 / 出现率」，现在同一张图同时带多个变体标签（鳴潮+鸣潮+WutheringWaves）时只计一张；小结句「最常出现的标签」也按归一后的名字合并（原先显示繁体原文）。
- 纸匣里的标签从此按「规范名」算账：筛选笺、搜索、统计标签云、年度报告的兴趣坐标都把同一标签的变体（如「鳴潮」「WutheringWaves」「wuthering waves」）并成一个，筛选一笺全中、搜索「鸣潮」也能命中只带「鳴潮」的藏品、计数合并不再分裂。
- 纸匣筛选收进一张「筛选纸」：搜索框下只剩一枚筛选钮、已选笺（点一下单独撤掉）和结果计数；打开后桌面是纸片弹层、手机是底部抽屉，同一份内芯选来源 / 作者（可搜索、带张数）/ 标签（纸内搜索，默认 40 个）/ 收入月份 / 未读 / 今日去年。页上不再摊站点 Chip 行和作者下拉。
- 纸匣筛选标签超过一行时默认折成一行，点「展开」才铺开；已选标签排在最前，清空/展开按钮不跟着被裁掉。
- 案头去浏览改成当前站默认流的封面铺纸（Pixiv 登录后用推荐、图站用最新、FANBOX 用首页），不再是一张空字卡；未读纸叠改叠真封面并放大张数；未拆信带上画师头像。今日报纸仍是日榜四格。

### 新增

- 纸匣可以批量整理标签了：点「选择」进入勾选模式（点卡片勾选、支持全选），底部工具条对选中的藏品**一次加标签 / 删标签**——删标签按归一后的名字匹配（勾「鳴潮」连「鸣潮」变体原文一起删），加标签写规范名；动作前显示「影响 N 张」（只计真会改动的）。本地写回不会被远端同步刷掉。
- 案头会自己动了：去浏览铺纸（默认流封面凑得满两批时每 8 秒整批淡换）、今日报纸（改成长长一条**水平慢速滚动**，日榜整版首尾相接循环播放，悬停停一停）、关注画师墙（Pixiv 登录时右栏新增，关注画师最新更新的作品图随机一批每 8 秒整墙淡换，点格子直达作品；凑不满就静态展示现有几张）都是系统自动轮播，没有手动切换；切后台或滚出屏幕会暂停，系统开了「减弱动态效果」则静止。
- 宽屏适配：大屏（≥1536px）案头整体放宽，去浏览铺纸变 6 列 12 格、画师墙变 4 列，单屏能看到的图更多，不再收在一小块里。
- 设置 → 存储新增「标签整理」卡：把大小写 / 全半角 / 空白这类机械变体自动聚成簇，点一下归并到规范名；繁简、中英这类跨语言变体（如「鳴潮」与「WutheringWaves」）搜出来手动归并。归并只记别名对照、不动藏品本身，删掉别名即还原；标签数据随备份与多端同步走，旧备份不含该字段时不会清空本地。
...
对应 [PR #142](https://github.com/Atsukiizumi/Kami-paperbox/pull/142) ~ [PR #152](https://github.com/Atsukiizumi/Kami-paperbox/pull/152) 与本 PR（媒体守卫 fake-ip 修复）。

### 新增

- 回顾三件套 · 今日去年：纸匣页会在「去年的今天」收过图时递上一枚笺条，点开即把列表过滤到那批藏品（可关闭的「今日去年」筛选片，复用整页瀑布流）；前年、大前年的同日命中也一并按年份归组计数。
- 回顾三件套 · 年度报告：统计页新增「年度报告」长页——选一个年份，读出那一年的开篇大数（收了多少张、最勤的月份）、心头好画师 Top 3、兴趣坐标 Top 5 标签、收藏节奏（峰值相位与最爱周几）、之最（最大的一张、页数之最、收藏跨度）和一句只从数据推导的结束语；全部在本机从纸匣元数据算出，读不出的字段直接留白不编造，整页不套滚动渐显、打印 / 截图不白屏。
- 回顾三件套 · 随手翻一张：纸匣页头部新增翻牌对话框，随机翻出一张藏品的封面与题注；「再翻一张」带纸面翻转动效（rotateY 过渡，`prefers-reduced-motion` 下直接换牌），「看详情」直达作品页；翻牌池只收翻得出封面的条目，卡背是折页记号花纹。
- 画师名称整理：同一画师的不同写法（`@handle`、表情装饰、改名）不再被当成多个人——导出 ZIP 分夹、镜像路径 `{author}`、统计画像与纸匣作者筛选现在按画师 ID / 规范化名归一（不动任何已保存的文件与文件夹）；设置 → 存储 新增「画师名称整理」卡片，自动聚出待归一的名称簇，挑一个规范名（或手输）即可统一，别名随账号同步与备份往返。
- 统计页整页重设计：hero 数字带（四项大数 count-up）→ 用户画像（收藏节奏时钟 + 一句画像语、心仪画师 Top 10、兴趣标签纸片云、来源构成堆叠条、画像小结语）→ 明细统计（按月时间线 / 存储占用 / 查重状态）三层版式；画像聚合全部在本机用纸匣元数据算出，空纸匣给宣纸空态引导，`prefers-reduced-motion` 下动效归零。
- 设置 → 存储 新增备份提醒：距上次成功导出超过 30 天会出现笺条提示（从未备份过则显示「尚未备份过」的一行提示）；时间戳只记本机，不进备份文件与服务端同步。
- 设置 → 说明 新增「上游健康」卡片：按站点显示近期请求成功率、延迟中位数与最近失败时间（30 秒刷新），上游抽风时不用再靠感觉猜。纯内存统计，不影响请求链路，仅登录用户可见。

### 变更

- Docker 部署支持一键升级：main 每次合并自动构建镜像到 GHCR（`ghcr.io/atsukiizumi/kami-paperbox`，打 `v*` 标签时额外出版本 tag），已有部署 `docker compose pull && docker compose up -d` 即可升级；本地 `--build` 路径不变。
- 服务端日志升级为结构化输出：默认一行人类可读（保留 `[模块:操作]` 标签），设 `KAMI_LOG_FORMAT=json` 可切 JSON 行；每条日志自动携带请求 id，API 响应头同步返回 `X-Request-Id`，报障时提供这个 id 即可精确对账。

### 修复

- **代理（fake-ip）环境下未缓存的图片全部裂开**：媒体防私网守卫把 198.18.0.0/15 保留段当私网拦——而它正是 Clash 系代理 fake-ip 模式的默认映射段，本机所有图站域名都解析到这一段（实测 i.pximg.net 等 → 198.18.0.x），缓存命中的图不走守卫所以正常，未缓存的一过闸就被拒。现在：fake-ip 段视为「代理托管解析」放行；且配置了出站代理时整道解析闸跳过（连接由代理侧解析，本机 DNS 答案不代表连接目标）。已实机验证：代理环境下未缓存图片恢复 200，不存在的路径透传上游 404（此前死在 400 非法地址）。
- **智能文件夹与画师追踪列表此前从未真正进入账号同步与备份文件**（采集白名单漏采），且旧版本数据拉取时会用空列表把本地清空——现在随设置段正常同步/备份，升级到本版本后首次拉取也不会丢本地列表（字段缺失时保留本地；显式清空仍照常生效）。另修复：智能文件夹在画师改别名后可能解析不到作者、误显示全部藏品——保存时一并记录稳定的作者簇键。
- 登录 / 设置页里站点身份获取（昵称头像）与登录状态同步失败时，现在会弹明确提示「请稍后重试」，不再静默吞掉失败让人以为已成功；同一失败连续触发只显示一条提示，不刷屏。成功流程行为不变。
...
- 纸感质感批（三）· 日历与移动抽屉：Pixiv / booru 榜单的日期选择与纸匣「按月筛选」从原生输入框换成纸感日历面板（按钮弹出、今天宣纸色、选中墨点、一键回到最新/全部时间，按月筛选时整月高亮）；手机上设置页的横向分类条换成「分类」按钮 + 底部抽屉（跟手拖拽收起），桌面分类列表不变。
- 纸感质感批（二）· 手绘风格（可切换）：设置 → 外观新增「界面风格」——默认「利落」，可切「手绘」：标题换霞鹜文楷手写体（分片按需加载，利落态零下载）、笺条（最新/置顶）改手写微歪边、浏览卡片悬停出现极淡墨晕与半度歪斜，另提供毛笔笔触下划线工具类。控件形态两种风格完全一致；选择随账号同步与备份往返；`prefers-reduced-motion` 下墨晕位移归零。
...
- 设置页 Pixiv / FANBOX 两个 Cookie 框彻底分开：各自独立分块、各有自己的「登录」按钮；粘进哪个框就只写哪个站（粘错站会提示换框），不再出现「一个框填了 Cookie，另一个框跟着填同一个值」。此前 Pixiv 登录 / 粘贴会把会话自动复制进 FANBOX 框（裸值粘贴还会两框同填）。FANBOX 框留空时照旧可用——请求层自动以 Pixiv 会话回退浏览公开内容，需要订阅内容时再单独登录 FANBOX。
...
对应 [PR #110](https://github.com/Atsukiizumi/Kami-paperbox/pull/110) ~ [PR #134](https://github.com/Atsukiizumi/Kami-paperbox/pull/134)。
...
- 访客模式：应用账号开启但未登录时，浏览、榜单、搜图、图片加载照常可用（修复前全部 401）；访客的图站 Cookie（Pixiv 等）只存在自己浏览器的本地存储、随请求携带，不落服务端——有 Cookie 就能用「为你推荐 / 关注」，没有则照旧提示需要登录。登录应用账号后设置与凭据同步服务端（原行为）。个人数据面（纸匣、会话同步、登录中转）和账号关闭形态的局域网令牌闸不受影响。
- 端到端测试框架（M7）：Playwright 收编关键路径（浏览→详情→入队→纸匣）、浏览缓存回归、访客层行为三条场景，CI 新增 e2e job 打真实上游（yande）验证。
- 队列三件套（M6）：设置里可调并发数（1-4，默认 1，同时处理多条下载）；失败自动重试——指数退避（4s/8s/16s，最多 3 次尝试），需要登录 / 订阅 / 内容不可用这类用户态错误不重试直接判死，重试期间队列页显示「失败 n 次后自动重试」；跨标签页共享——同一时间只有一个标签页实际执行队列（Web Locks 全局锁，执行者关闭后其他标签页自动接管），其余标签页通过 BroadcastChannel 实时镜像队列进度。
- 错误可观测性最小版（S7）：上游侧失败（网络 / 源站 5xx / 反爬拦截 / 数据不可解析）在 /api/source 回 502，用户态错误（需要登录、安全模式、参数不合法）保持 400——此前一律 400 掩盖了源站问题；服务端二十来处静默吞异常点（快照导出、缓存读写、配置加载、身份探测等）补上 `[模块:操作]` 前缀的 console.warn，排查时日志可对号入座。
- 纸匣智能库（B）：收藏可按标签 / 月份筛选，常用筛选条件可存成「智能文件夹」随账号同步；新增同图不同源查重——收藏入库即算 dHash 感知哈希，纸匣内可扫描重复组（相似阈值可调）、逐组处理或忽略本对，统计页如实显示哈希覆盖率；新增纸匣统计页（/vault/stats：来源 / 画师 / 标签分布、按月收藏时间线、存储占用、查重状态）。
- 画师更新追踪（A）：画师页可「追踪」，追踪页 /watch 汇总各画师新作数与最新缩略图，水位线标已读且跨设备同步；可一键导入 Pixiv 关注列表；红点入口在桌面侧栏与移动端顶栏铃铛（底部导航保持六项）；追踪上限可调（20~500），页内注明缓存占用说明。
- 画师页批量收藏（D）：Pixiv 画师页与 FANBOX 创作者页支持勾选批量入纸匣 / 下载，「加载至 200 张」自动翻页聚合，单次入队上限 200；已在纸匣或队列中的作品自动跳过并提示数量。
- 纸匣导出 ZIP（E）：纸匣页按当前筛选结果一键导出——服务端流式打包（fflate 边压边吐，不整包进内存），按画师分文件夹；缺图条目跳过并写入包内 `_skipped.json`；预估超 500MB 时提示分批。
- 多 P 预览滚轮切页：大预览浮层打开时滚轮下/上滚即翻下一/上一页（循环）；阈值 + 时间锁防触控板惯性连翻，浮层关着不挂监听、网格滚动不受影响。
- 动效批：主题切换改为从点击点圆形扩散揭示（View Transitions，不支持的浏览器/reduced-motion 直接切换）；瀑布流卡片进视口才渐入显形（transform/opacity only）；统计页数字滚动、空态纸标漂浮、队列完成一次性弹跳——全部响应 prefers-reduced-motion，零新依赖。
...
- 访客现在也能走「登录 Pixiv / FANBOX」的浏览器登录中转（TD-20）：此前未登录应用账号时中转接口 401，登录对话框永远转圈（主画面「正在打开官方登录页」、脚注却写「已结束」），只能手动贴 Cookie。现在登录中转、HttpOnly 会话镜像、Pixiv/FANBOX 身份解析三条通道对访客放行——访客的 FANBOX / R-18 图片恢复显示，贴了 Cookie 头像昵称也能解析；登录对话框遇到网关错误会立即报错退出，不再空转。
- 账号同步不再覆盖访客期新录的图站 Cookie（TD-21）：登出期间换绑的 Pixiv Cookie 此前会在登录瞬间被服务端旧档整段冲掉、还被当作新改动固化回去。现在访客期的凭据改动记脏标记，登录拉取时本地非空凭据保留（新设备首登的恢复流不受影响），并在拉取后补推一次把服务端补齐。
- 快照恢复失败的行不再静默丢失（TD-22）：数据库快照个别行恢复失败（schema 漂移、约束违约）时，旧实现只打一行日志就继续——紧接着的快照回写会把失败行永久冲掉，丢的可能是登录会话。现在失败行先重试一轮，仍失败的原样隔离到 `.data/kami-db-snapshot.restore-failed.json` 供人工处置。
- 认证端点限速（SEC-11）：注册 / 登录等 `/api/auth/*` 端点此前完全开放，局域网内任意设备可无限注册账号写库；现在 60 秒 20 次，超出回 429。
- `/api/account/sync` 补上与其他数据面路由一致的跨站校验（SEC-12）：此前是唯一不做 Fetch-Metadata 同站检查的认证路由，只靠 SameSite cookie 兜底。
- 修 Pixiv 日榜「未公布窗口」404：pixiv 的日榜公布时间晚于 JST 午夜，公布窗口内连「日历昨天」这个日期本身都会 404——此前显式选了这种日期（或带日期的历史缓存补刷）直接报错。现在服务端对「今天/未来」的日期一律按未指定处理，对「昨天/更新」日期的 404 视同未公布自动回落，回落目标改为无日期请求（pixiv 自己返回最新已公布一期，任何时刻有效）；翻页与第一页回落目标保持一致，更早的历史日期 404 仍如实报错。
- `next build` 不再触碰真实运行数据：构建期的页面数据收集会加载数据库模块并触发「快照恢复 + 回写」，等于在构建机上执行了一次真实数据迁移——本次本地账号凭据丢失事故的直接载体。现在 `phase-production-build` 阶段完全跳过数据库引导。
- 端到端测试的数据彻底隔离：e2e 注册的测试账号、快照、媒体缓存全部落在系统临时目录（KAMI_ROOT 指到仓库外并复制 migrations），不再写进开发者的真实 `.data`。
- 修 Docker 构建失败（`/pnpm-lock.yaml: not found`）：`.dockerignore` 还带着 npm 时代排除 pnpm-lock 的旧条目，TD-06 统一 pnpm 后没同步删掉，导致锁文件进不了构建上下文、`COPY package.json pnpm-lock.yaml .npmrc` 必挂。CI 不构建镜像所以此前未暴露。
...
- 手机端浏览布局：窄屏改单列统一满宽——每卡占满行宽、高度随原始长宽比（上限钳制），不再走行拼版留半屏空；桌面排布不变。

...
import { readKamiConfig } from "./kami-config.mjs";
...
test("ensureKamiConfig copies the example and never overwrites", () => {
  const root = mkdtempSync(join(tmpdir(), "kami-cfg-"));
  writeFileSync(join(root, "kami.config.example.json"), '{"host":"127.0.0.1","port":3000,"proxy":""}\n');
  ensureKamiConfig(root);
  assert.equal(JSON.parse(readFileSync(join(root, "kami.config.json"), "utf8")).port, 3000);
  writeFileSync(join(root, "kami.config.json"), '{"host":"0.0.0.0","port":8080,"proxy":"http://127.0.0.1:7890"}\n');
  ensureKamiConfig(root);
  assert.equal(JSON.parse(readFileSync(join(root, "kami.config.json"), "utf8")).proxy, "http://127.0.0.1:7890");
});

...
import { applyKamiEnv, ensureKamiConfig } from "./kami-config.mjs";
...
export function resolveSpawn(command, args, root = projectRoot()) {
  ...
  const local = resolveLocalBin(command, root);
...
export function missingDepsHint(command) {
  ...
  const lines = [`找不到命令「${command}」。`];
  if (!installed) {
    lines.push("还没有安装依赖。在项目目录执行：");
  } else {
    lines.push("依赖不完整。请重新安装：");
  }
...
function main(argv) {
  ...
  const env = applyKamiEnv(
    withLocalBin(mergeAppEnv(readAppEnv(root), process.env)),
    root,
  );
  const run = resolveSpawn(command, args);

 ...
 * 案头画师墙。
 *
 * 作用：Pixiv 登录时，右栏用关注画师的**最新作品缩略图**砌一面 3 列方格墙——
 *      洗牌成批（9–12 张）、每 8s 整墙交叉淡换；点格子进作品页。
 *      3–8 张不硬凑，静态展示现有几张；少于 3 张砌不成墙，整块隐身。
...
export function DeskArtistWall() {
  ...
  const frames = useMemo(() => {
    // 9 张以上按动态批量（9–12）成帧轮播；3–8 张不硬凑，静态展示现有几张。
    if (pool.length < 3) return [];
    const size = pool.length >= 9 ? artistWallSize(pool.length) : pool.length;

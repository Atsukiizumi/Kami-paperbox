 ...
 *   `/auth/popup` (`app/auth/popup/route.ts` + `popup.server.ts`) — 302s to the
 *   broker/upstream login (no app chrome) and,
 ...
 * handler (no await before this). The path is the Next route at
 * `app/auth/popup/route.ts` — not a React page.
 *

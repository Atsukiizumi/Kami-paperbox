...
> 基线 `main@33abd67`（2026-09-17 校准，含 #136~#153 路线/统计/智能库二期/fake-ip 热修、**#154~#160**：案头稿纸版式重设计、标签别名归一层、案头轮播三件套+宽屏、纸匣批量加删标签、备份守卫与 Turbopack 清零、查重鸽笼分桶与按张去重口径、案头精修）。发版 **0.10.8**（2026-09-17，tag v0.10.8，收编 #154~#159）；#160 案头精修属 Unreleased。
...
| 版本 | **0.10.8**（2026-09-17 发版，tag v0.10.8；Unreleased = #160 案头精修：报纸贪吃蛇流动、画师墙固定 3×3） | CHANGELOG.md |
...
| 部署 | 本机 `pnpm dev / build+start`；Docker（node:24-bookworm-slim 两阶段 + compose 卷）；LAN 令牌形态；Vercel 已正式移除（#170，D4——官方形态收敛本机 + Docker） | Dockerfile |
...
│   ├── */page.tsx          #   15 个页面壳（案头占 /、浏览迁 /browse；含 watch、vault/stats、vault/report）：re-export src/routes/* 实现
│   ├── layout.tsx          #   唯一 server 组件（文档骨架 + 主题防 FOUC 内联脚本）
│   └── providers.tsx       #   Provider 嵌套树（Auth > Query > Theme > Tooltip > 壳组件）
├── src/
│   ├── routes/             # 页面与 API 处理器真实实现（TanStack 风格命名，含 desk/vault-stats/vault-report）
│   │   └── api/            #   13 个 API 处理器（zod 校验、502/400 错误分类；account/sync 与 vault/stats/storage 两条内联在壳层）
│   ├── components/         # 46 个顶层组件（app-shell/masonry-board/artwork-card/vault-filter/vault-batch-tags/use-batch-selection…）+ settings/ 分区组件 11（9 分区 + 画师名称整理 + 标签整理）+ desk/ 案头子域 8（browse-sheet/newspaper/letters/stack/artist-wall 五组件 + use-crossfade/use-viewport-active/use-media-flag 三 hook）+ stats/ 统计组件 7 + ui/* 基件 22
│   ├── lib/                # 全部业务与基础设施（119 文件，.server.ts 后缀 = 服务端专属）
...
│   │   └── （平铺）        #   其余 = ui-support 域：masonry/watch/queue/tag-*/desk-*/vault-tag-alias/theme/parse...（既定形态）
...
├── docs/                   # 本套工程文档（2026-09-17 起随 git 追踪）+ 专题文档
...
flowchart TB
  ...
  subgraph kami["Kami 纸匣（自托管 · Next.js 16 单进程 · 0.0.0.0:8080）"]
    app["Web UI（案头/浏览/搜索/纸匣/队列/追踪/设置）<br>+ 15 条 /api/* 数据面"]
  end
...
| 层 | 位置 | 内容与边界 |
| --- | --- | --- |
| UI / Presentation | src/routes/*、src/components/* | 页面 15 + 组件（顶层 46 / settings 分区 11 / desk 子域 8 / stats 7 / ui 基件 22）；ProxiedImg/artwork-card/app-shell 为枢纽 |
...
**依赖方向总体单向**（UI → client lib → API → server lib → infra → storage），反向依赖基本没有；`app-shell.tsx` 对浏览页已 next/dynamic 拆包（PER-13 ✅，PR #119；#154 后拆包对象随迁 `@/routes/browse`），单向性恢复。
...
| 动效 | src/lib/motion.ts FLIP + View Transitions 主题揭示 + IntersectionObserver 进视口渐入；全部响应 prefers-reduced-motion；合成线程优先（transform/opacity only）。案头轮播三件套（#156/#160）同纪律：去浏览铺纸/画师墙 8s 整批交叉淡换（use-crossfade 定时链 + 双缓冲）、今日报纸贪吃蛇 rAF 只写 transform（纯合成层），减弱动态效果 = 全部静止（淡换锁首帧、蛇静止渐隐行） | #134 动效批落定基线 + 案头轮播扩展 |
...
| 账号同步与加密 | 四段 LWW 同步、KEK 信封加密 | sync/account-sync.ts、storage/crypto-box.ts | user_sync_* 3 表 | backup-client |
...
| 纸匣查重与统计 | dHash 感知哈希、鸽笼分桶配对（#159）、并查集聚类、按张去重口径的存储聚合 | storage/dhash.ts、vault-dedup.ts、vault-profile.ts | vault_hash / vault_dup_dismissed | vault-store |
| 案头主页 | 打开应用的稿纸主页：去浏览铺纸/今日报纸/未拆信/未读纸叠/关注画师墙/去年今日笺，轮播三件套与蛇形报纸的纯函数底座 | routes/desk.tsx、components/desk/*、lib/desk-carousel.ts（+desk-newspaper/desk-preview/desk-unread） | localStorage 历史窗（未读 90 天） | source、vault、watch-badge |
| 标签别名归一 | 标签机械/跨语言变体并成规范名：单跳别名应用（禁链）、簇聚类、批量加删的增删集线 | lib/vault-tag-alias.ts、storage/vault-query.ts（haystackOf/vaultTags/filterVaultItems）、components/vault-batch-tags.tsx | 设置段 tagAliases（≤500 条） | vault-profile、vault.tsx、vault-stats/report |
...
| 定时/防抖 | 快照 dump 500ms 防抖 + 30s 看护；水位每 50 写扫；联想 200ms；同步推送 4s；案头轮播 8s 定时链（setTimeout 链，暂停后恢复重起整拍不追赶） | sweepCache 已分批 yield（PER-11 ✅ PR #120） |
...
**问题**
1. `src/lib` 仍有平铺区（ui-support 域，watch/masonry/tag/desk 等 72 文件），仅 upstream/auth/storage/sync 有目录；eslint 双边界（M10）已拦 server 误引与反向依赖，但域内横向依赖仍靠自觉。
2. ~~巨文件集中在 UI 层~~ #147 已拆：settings 900→300（九分区子组件 `settings/*`）、artwork-card 637→305（交互 hook + 托盘/题注/栅格）；browse.tsx 776 行（原 index.tsx，案头改版后浏览页迁 /browse）成为最大单体（见 05 号 UI-BIG 尾项）。

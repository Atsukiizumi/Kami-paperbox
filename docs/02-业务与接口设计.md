...
> 基线 `main@33abd67`（2026-09-17 校准，含 #136~#160）。吸收原 02-需求规格说明 / 04-业务流程说明 / 07-接口设计说明 / 核心业务流程图 / 时序图 / 状态图 / upstream.md / pixiv-auth.md 精华（编号原件已删除；upstream/pixiv-auth 保留为专题文档）。本项目无需求文档存档，功能清单从代码、页面、接口与数据库反向还原。
...
| 编号 | 功能 | 角色 | 入口 | 核心逻辑 | 验证 |
| --- | --- | --- | --- | --- | --- |
| F-01 | 多源浏览（瀑布流） | 全部 | `/browse`（案头「去浏览」进） | 三源 feed：Pixiv 8 榜单 mode + 推荐 + 关注、FANBOX 主页/赞助/创作者、Booru recent/hot/日周月榜；本地 50/页分页 + 无限补页；过滤前翻页判定；**手机 <480px 单列满宽**（#133：高度随原始长宽比，clampAspect 0.45~3.2） | 单测+e2e |
| F-02 | 搜索 | 全部 | `/browse`、搜索框 | 文本搜索（Pixiv filter 模型 safe/ai/排序/日期、booru 标签规范）+ 联想（本地+远端 200ms 防抖）+ 保存标签 | 单测 |
...
| F-08 | 纸匣管理 | 主人 | `/vault` | 本地 IDB + 服务端目录合并（mergeVaultItems：本地 tags/title 优先、hasFile 以远端为哨兵，#157 抽出并测试锁）、**筛选纸**（vault-filter：搜索框下只留筛选钮 + 已选笺 + 计数，桌面纸片弹层/手机抽屉同芯选来源/作者可搜索带张数/标签纸内搜索默认 40 个超行折叠/月份/未读/今日去年——取代站点 Chip 行与作者下拉，#154）、导出/移除、分批渲染 | 单测+组件测试 |
...
| F-16 | 内容过滤 | 全部 | 设置页逐站开关 + 浏览页当前站快捷开关（#162） | safeModeBySite（persist **v12→v13**，五站各自 R-18 拦截；请求仍按 op 带单个 safeMode 布尔，danbooru 强制附加 rating:g TD-24 ✅）、hideAi（Pixiv 专属：aiType/标签，关闭时卡牌打 AI 标）、22 个未成年相关 BLOCKED_TAGS 无条件拉黑 | 单测锁 |
| F-17 | 标签词表 | 本地 | 设置页 | 内置 1,104 条 en→zh + 浏览自动收录 + 用户覆盖三级合并；导入导出/从纸匣扫入 | 单测 |
| F-18 | 文件夹镜像 | 本地 | 设置页 | FS Access API 授权目录（文件夹名 folderLabel 随账号同步段走，句柄只在本机 IndexedDB——跨设备要重选），下载/镜像双开关、路径模板（token 插入+预览）、hash 扫描检测外部替换；**启动预检（#163）**：folderHealth() 只 query 不 request，none/prompt/denied/unreadable 四级病因 → toast 提示带「去设置」直达存储分区 | — |
...
| F-22 | 纸匣智能库 | 主人 | `/vault`、`/vault/stats` | 标签/月份筛选 + 智能文件夹（随账号同步）；dHash 64-bit 入库即算、汉明距离 ≤10 并查集聚类（**候选对生成改鸽笼分桶**：dhash 均分 4 段各建索引、距离 <16 位必共享一段全等只在桶内配对，4000 条 5.5s→7ms；阈值 ≥16 位回退朴素保语义，#159）、忽略对持久化、候选组现场重算；**标签计数按张去重口径**（同图双变体只计一张、topTag 走别名归一，#159）；统计页三层版式（hero 数字带 count-up → 用户画像：节奏时钟/画像语/心仪画师/标签云/来源构成/小结语 → 明细统计：月时间线/存储占用/查重状态；#150 重设计，聚合全在本机 vault-profile 纯函数） | 单测 |
...
| F-29 | 案头主页（**Pixiv 专属**，#163） | 全部 | `/`（打开应用落地；浏览迁 /browse） | **非 Pixiv 四站（FANBOX/三图站）无案头**：切站直达 /browse、`/` 水合后重定向。稿纸版式（#154/#156/#160 定稿）：①去浏览铺纸——当前站默认流封面铺满主纸（Pixiv 登录用推荐、booru 用最新、FANBOX 用首页），前 3 批整批交叉淡换 8s（批随断点：默认 4×2 八张、2xl 起 6×3 十八张，池深恒 3 批 = 24/54，sheetBatch(is2xl)；不足两批静态）；②今日报纸——当前站日榜全量 cap 30 化作小矩形节点**贪吃蛇式流动**：蛇头左→右平滑行进（rAF 只写 transform 纯合成层）、尾迹透明度逐级递减（snakeOpacity 下限 0.3）、出右回左无缝回卷；蛇长随图片量伸缩（snakeNodeCount 封顶路径容量）后按 slot 环换班全名次轮流当蛇头；悬停/切后台/滚离视口冻结；reduce 静态渐隐行无滚动条；③未拆信（有追踪才出现，只读上次角标不现场检查）；④未读纸叠（90 天窗内收后未开，六张扇形贴边均分）；⑤关注画师墙——Pixiv 登录时右栏**固定 3×3 九格**（格数恒 9、缩略图随卡片宽度伸缩，关注新作品流作品图，洗牌成批 8s 整墙淡换；<3 张整块隐身、3-8 张静态）；⑥去年今日笺；⑦宽屏 2xl 容器放宽 90rem。案头不预取浏览页、报纸独立 queryKey 不与保活浏览抢键 | 单测（desk-carousel/desk-newspaper/desk-preview/desk-unread + hook 组件测试）+ e2e（critical-path 断言 yande 落 `/` 直达 /browse）+ 目检 |
| F-30 | 标签整理（别名归一） | 主人 | 设置→存储「标签整理」卡 | `vault-tag-alias.ts` 纯模块（#155 tag-tidy M1）：normalizeTagKey（大小写/全半角/空白机械归一）+ applyTagAlias **单跳禁链**（A→B、B→C 时 A 只到 B，不递归）+ clusterTagVariants（机械变体自动聚簇、跨语言变体搜索手动归并）+ parseTagAliases（≤500 条、键值各 ≤120 字）；消费点：vaultTags/filterVaultItems/haystackOf（vault-query 筛选与搜索双侧）、tagCloud/reportNarrative/profileSummary（vault-profile）、vault.tsx cardFromMeta（卡片展示规范名）、vault-stats/vault-report；归并只记别名对照不动藏品，删别名即还原；tagAliases 进设置 store（persist **v11→v12**）与备份三处接线，applySegment「字段缺失保留本地」守卫 | 单测 + 组件测试 |
| F-31 | 纸匣批量加删标签 | 主人 | `/vault`「选择」模式 | tag-tidy M2（#157）：useBatchSelection hook（从 creator/user 两页抽取行为零变化）+ artwork-card selection.toggleOnCardClick + BatchToolbar 加 children/label/max（vault 传 max=null 解除批量上限）；加标签写规范名（applyTagAlias）/ 删标签按并集点选、归一匹配变体原文同删；「影响 N 张」只计真会改写的条目；本地 IDB 写回不被远端同步刷掉 | 单测 + 组件测试 |

**体验层基线**（#134 动效批，全部响应 prefers-reduced-motion、零新依赖）：masonry 子卡进视口才显形（共享 IntersectionObserver + data-inview，transform/opacity only）；统计页数字 count-up（use-count-up，rAF 600ms ease-out）；空态纸标漂浮；队列完成一次性弹跳；案头轮播三件套（#156/#160：整批交叉淡换 / 整墙淡换 / 蛇形 transform 流动，暂停三条件 = document.hidden / 滚离视口 / 帧数不足，减弱动态效果全部静止）。**暂缓项**：路由共享元素转场（React 19.2 stable 未导出 ViewTransition，见 06 号路线）。
...
- **调用链**：`routes/browse.tsx useInfiniteQuery` → `fetchSource`（POST /api/source）→ `withDataPlane` 闸 → `routes/api/source.ts` → `cachedDispatchFetch` → `unstable_cache` 二层 → `dispatchFetch` → 站点适配 → `outboundFetch`（undici 池→curl 降级）→ 上游。
...
| 流程 | 触发 | 处理 | 异常与回滚 |
| --- | --- | --- | --- |
| **查重（F-22）** | 收藏写入即算哈希；`/vault` 扫描 | dhash（jpeg-js/pngjs 解码 → 9×8 灰度 → 64-bit 差分；webp/失败不落行）→ POST scan 补算缺失（每 50 条 yield）→ clusterDupes 并查集（≤10；候选对生成**鸽笼分桶**：dhash 4 段索引桶内配对，阈值 ≥16 位回退朴素，#159）→ 排除忽略对 → 候选组现场重算不落库 | 扫描幂等；大库存分批不阻塞；无聚合状态无需回滚 |
...
| 案头 | desk.tsx → desk-preview/desk-newspaper（fetchSource 现拉，queryKey 独立不吃保活缓存）+ listVault/listServerVault + unreadItems/onThisDay（本机）+ fetchSource pixivFollowing（画师墙） |

...
> 基线 `main@022c064`（2026-09-15 校准，含 #126 迁移 0004；#136~#153 无库结构变化——浏览器端 kami-settings v11 增 uiStyle、lastBackupAt（只记本机）、authorAliases（随同步/备份往返）字段）。吸收原 06-数据库设计说明 / ER图 / 17-备份与同步加密设计 / storage.md 精华（编号原件已删除；storage.md 保留为专题文档）。依据优先级：数据库实际结构（migrations/）> 迁移脚本 > 业务代码推导。
...
- localStorage：`kami-settings`(v11，含凭据——SEC-03 长期残余；v11 增 uiStyle #139、lastBackupAt #148（只记本机不进同步段）、authorAliases #151（≤200 条，随同步/备份往返）、smartFolders/watchArtists/watchLimit 自 #151 起真正进同步段与备份——此前漏采白名单)、`kami-queue`、`kami-history`(v2)、`kami-tag-lexicon`、`kami-tag-catalog`、`kami-browse-v2`(24h，键含凭据指纹 SEC-04)、`kami-account-sync-v2`、`kami-cred-dirty-v1`（脏标记）、watch 角标。
...
**加密原语**（src/lib/crypto-box.ts）：PBKDF2-HMAC-SHA256 210,000 轮 + AES-256-GCM（口令错=解密失败，自带完整性）；WebCrypto 原生，Node/浏览器同构。

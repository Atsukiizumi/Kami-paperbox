...
> 基线 `main@32a1e09`（2026-09-21 校准，含 #126 迁移 0004；#136~#162 无库结构变化——浏览器端 kami-settings v13 增 uiStyle、lastBackupAt（只记本机）、authorAliases（≤200 条）、tagAliases（≤500 条，#155）、safeModeBySite（#162，每站点 R-18；旧全局 safeMode 广播迁移）字段，均随同步/备份往返）。吸收原 06-数据库设计说明 / ER图 / 17-备份与同步加密设计 / storage.md 精华（编号原件已删除；storage.md 保留为专题文档）。依据优先级：数据库实际结构（migrations/）> 迁移脚本 > 业务代码推导。

...
- [ ] 失败测试 → 实现 → 通过 → commit `feat: 画师追踪数据模型与解析`
...
- [ ] 失败测试 → 实现 → 通过（全量 pnpm test）→ commit `feat: pixivMyFollowing 关注列表 op`
...
- [ ] 失败测试 → 实现 → 通过 → commit `feat: 追踪检测编排（并发 2 + 失败隔离）`
...
**要点：**
- [ ] /watch：检查更新（Task 3 编排）、行内新作数/失败态/最新缩略图、单个与全部标已读、导入 pixiv 关注（分页拉全、去重合并、超限截断提示）、存储占用注记
- [ ] 红点角标 = watch-store 汇总（检查后更新，localStorage 持久化）
- [ ] Playwright 冒烟：/watch 渲染 + console 干净（未登录态优雅提示）
- [ ] commit `feat: 追踪页/画师页 toggle/红点入口`

### Task 5: 收尾

- [ ] `pnpm typecheck && pnpm test && pnpm build` 全绿（显式退出码）
- [ ] docs：CHANGELOG Unreleased、docs/05 追加小节、spec 状态改已实现、plan 勾选
- [ ] 分支 `feat/artist-watch` 开 PR，CI 绿后合并（用户口径）
